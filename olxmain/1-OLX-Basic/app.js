...
app.post("/ad/:id",urlencodedParser,(req,res) => {
    ...
    newMessage.sender_id = req.session.user._id
    newMessage.message = req.body.msg
    newMessage.adId = req.params.id
    adModel.findOne({id: req.params.id}, (err,doc) =>{
        newMessage.receiver_id = doc.postedBy
...
app.get("/user/ad/:id/chats", (req,res) => {
    messageModel.find({adId: req.params.id}, (err, docs) => {
        docs = _.groupBy(docs, "sender_id")
        docs = _.map(docs, (value) => { return value })
